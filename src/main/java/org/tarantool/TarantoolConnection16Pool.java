...
public class TarantoolConnection16Pool implements TarantoolConnection16 {
    ...
    List<TarantoolConnection16ThreadSafe> pool = new ArrayList<TarantoolConnection16ThreadSafe>();

    public TarantoolConnection16Pool(String host, int port, int poolSize) throws IOException {
        this.poolSize = poolSize;
        for(int i = 0; i < poolSize; i++){
            pool.add(new TarantoolConnection16ThreadSafe(host, port));
        }
    }

    public TarantoolConnection16ThreadSafe connection() {
        return pool.get(random.nextInt(1000000) % poolSize);
    }

    @Override
    public void auth(String username, String password) {
        for(TarantoolConnection16ThreadSafe con : pool){
